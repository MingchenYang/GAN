...
Gaussian_dir = 'S:/pix2pix optflow/Train256/training_Gaussian/'
...
Gaussian_path = [os.path.join(Gaussian_dir, i) for i in os.listdir(Gaussian_dir)]
...
def train():
    ...
    # Slice the input gaussian path string, get a datset about string
    path_gaussian_ds = tf.data.Dataset.from_tensor_slices(Gaussian_path)
    # Load all Gaussian images into dataset
    gaussian_ds = path_gaussian_ds.map(read_and_load, num_parallel_calls=AUTOTUNE)

    # Pack images and labels together
    image_label_ds = tf.data.Dataset.zip((image_ds, label_ds, gaussian_ds))
    ...
    for epoch in range(Training_steps):
        for data, label, gaussian in ds:
            with tf.GradientTape() as Tape:
                # gen_input = label
                gen_input = layers.concatenate([gaussian, label], 3)
