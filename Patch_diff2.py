...
Result_dir = 'S:/pix2pix optflow/Test256/Unet_Mosaic_test/'
Label_dir = 'S:/pix2pix optflow/Test256/training/'
Output_dir = 'S:/pix2pix optflow/Test256/Unet_Mosaic_reverse_test_mask/'

Result_reverse_dir = 'S:/pix2pix optflow/Test256/Unet_reverse_test/'
Label_reverse_dir = 'S:/pix2pix optflow/Test256/label/'
...
def train():
    ...
    for num_sequence in range(Num_video):
        for num_image in range(Num_image_per):
            ...
            Result_reverse_path = Result_reverse_dir + Result_name
            Label_reverse_path = Label_reverse_dir + Result_name

            Result = read_and_load(Result_path)
            Label = read_and_load(Label_path)
            Result_reverse = read_and_load(Result_reverse_path)
            Label_reverse = read_and_load(Label_reverse_path)
            ...
            Result_reverse = Result_reverse / 255.
            Label_reverse = Label_reverse / 255.
            Result_reverse = np.reshape(Result_reverse, newshape=[L_node, W_node])
            Label_reverse = np.reshape(Label_reverse, newshape=[L_node, W_node])
            ...
            for L in range(L_times):
                ...
                for W in range(W_times):
                    ...
                    Result_reverse_patch = Result_reverse[L_start: L_end, W_start: W_end]
                    Label_reverse_patch = Label_reverse[L_start: L_end, W_start: W_end]
                    # Patch_diff[L, W, num_image] = np.mean(np.abs(Result_patch - Label_patch))
                    Patch_diff[L, W, num_image] = mean_squared_error(Result_patch, Label_patch) * 0.8 + mean_squared_error(Result_reverse_patch, Label_reverse_patch)
        ...
        index = np.int(L_times * W_times * Num_image_per * (1 - math.pow(0.5, 8)))  # Get the patch difference vector index
        print(index)
