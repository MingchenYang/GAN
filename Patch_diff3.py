...
def train():
    ...
    for num_sequence in range(Num_video):
        # Initialize patches difference
        Patch_diff = np.zeros(shape=[L_times, W_times, Num_image_per[num_sequence]])

        for num_image in range(Num_image_per[num_sequence] - 1):
        ...
        index = np.int(L_times * W_times * Num_image_per[num_sequence] * (1 - math.pow(0.5, 7)))  # Get the patch difference vector index
        print(index)
        ...
        for num_image in range(Num_image_per[num_sequence] - 1):
            ...
            beta = 1.0
            gamma = 0
            img_mask = cv2.addWeighted(Result_3C, alpha, mask, beta, gamma)
