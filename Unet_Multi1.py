...
File_dir = 'S:/UCSD_ped1/Train256/training_Multi/'
Output_dir = 'S:/UCSD_ped1/Train256/result_Multi/'
Save_dir = 'S:/UCSD_ped1/Train256/save_Multi/'
...
def build_generator():
    ...
    l5_2_1 = tf.reshape(l5_2[:, :, :, 0], shape=(-1, 1, LSTM_layer, LSTM_layer, 1))
    l5_2_2 = tf.reshape(l5_2[:, :, :, 1], shape=(-1, 1, LSTM_layer, LSTM_layer, 1))
    l5_2_3 = tf.reshape(l5_2[:, :, :, 2], shape=(-1, 1, LSTM_layer, LSTM_layer, 1))
    l5_2_4 = tf.reshape(l5_2[:, :, :, 3], shape=(-1, 1, LSTM_layer, LSTM_layer, 1))
    l5_2_5 = tf.reshape(l5_2[:, :, :, 4], shape=(-1, 1, LSTM_layer, LSTM_layer, 1))
    l5_2 = layers.concatenate([l5_2_1, l5_2_2, l5_2_3, l5_2_4, l5_2_5], 1)  # (None, 5, 256, 256, 1)

    l6_2 = layers.ConvLSTM2D(5, 3, 1, 'same', return_sequences=True)(l5_2)  # (None, 5, 256, 256, 10)

    l7_2 = layers.ConvLSTM2D(10, 3, 1, 'same', return_sequences=True)(l6_2)  # (None, 5, 256, 256, 20)

    # l8_2 = layers.ConvLSTM2D(10, 3, 1, 'same', return_sequences=True)(l7_2)  # (None, 5, 256, 256, 20)

    l9_2 = layers.ConvLSTM2D(5, 3, 1, 'same', return_sequences=False)(l7_2)  # (None, 256, 256, 10)
    ...
    l11_2 = layers.Conv2DTranspose(64, 3, 2, 'same')(l9_2)  # (None, 256, 256, 64)
