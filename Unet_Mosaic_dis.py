...
def Generator():
    # Encoder:
    input = tf.keras.Input(shape=[L_node, W_node, Channel * 4])
...
def Discriminator():
    input = tf.keras.Input(shape=[L_node, W_node, Channel * 5])
...
def train():
    ...
    generator = Generator()
    generator.build(input_shape=(Batch_size, L_node, W_node, Channel * 4))
    generator.summary()

    discriminator = Discriminator()
    discriminator.build(input_shape=(Batch_size, L_node, W_node, Channel * 5))
    ...
    for epoch in range(Training_steps):
        for data, label, mosaic in ds:
            ...
            if k % 1000 == 0:
                ...
                cv2.imwrite(Output_dir + str(k) + '.png', output_save * 255.)
            if k % 2000 == 0:
