...
Image_dir = 'S:/UCSD_ped2/Test256/label/'  # Image dataset in reverse file is the input of generator
Label_dir = 'S:/UCSD_ped2/Test256/training/'  # Label dataset in reverse is the real image
Output_dir = 'S:/UCSD_ped2/Test256/Unet_Reverse_test/'
Save_path = 'S:/UCSD_ped2/Train256/save_Reverse/66000Gmodel.h5'


def read_and_load(path):
    img = cv2.imread(path, 0)
    img = tf.reshape(img, shape=[1, L_node, W_node, Channel * 1])
...
def train():
    ...
    for name in os.listdir(Image_dir):
        ...
        output = np.reshape(output, newshape=[L_node, W_node, Channel * 1])
        output = output * 255.
