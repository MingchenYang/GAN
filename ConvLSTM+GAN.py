...
File_dir = 'S:/pix2pix optflow/Generator256/sequence/'
Output_dir = 'S:/pix2pix optflow/Generator256/CLSTM+GAN_result/'
Save_dir = 'S:/pix2pix optflow/Generator256/CLSTM+GAN_save/'
...
def lstm_gan():
    ...
    l5 = layers.Conv2D(1, 3, 1, 'same', activation='sigmoid', name='output_lstm')(l4)  # (None, 256, 256, 1)
    ...
    LSTM_Model = tf.keras.Model(input, l5, name='LSTM')
...
def train():
    ...
    lstm.build(input_shape=[Batch_size, Num_sequence, L_node, W_node, Channel])
    gen.summary()
    ...
    for steps in range(Training_steps):
        ...
        if k % 100 == 0:
            ...
            print("Step:{}, Generator Loss:{:.4f}, L1 Loss:{:.4f}, SSIM Loss:{:.4f}, Discriminator Loss:{:.4f}".format(k, g_loss, g_loss_l1, g_loss_ssim, d_loss))
            ...
            output_lstm = lstm(image_batch, training=False)
            output_lstm_save = np.reshape(output_lstm[0], newshape=[L_node, W_node])
            # print(output_lstm_save)
            cv2.imwrite(Output_dir + str(k) + 'lstm.jpg', output_lstm_save * 255.)
